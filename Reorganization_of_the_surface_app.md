...
**No special resampling at VPs (cusps).** It is tempting to densify the two CC half-curves meeting at a cusp, fearing their near-coincident image polylines weave. They do **not**, generically, and the genericity is the reason. A VP is a Whitney cusp — the codimension-0 stable singularity of plane-to-plane maps (the only ones, with folds) — whose local normal form is the semicubical arc `(s², s³)`. The two arms (`s>0`, `s<0`) leave the cusp along the **same** tangent direction but on **opposite** sides of the tangent line: the transverse-to-tangent coordinate is `s³ + O(s⁴)`, whose sign is `sign(s)`. This survives any target diffeomorphism `φ`, since `φ(s²,s³) = s²·a₁ + s³·a₂ + O(s⁴)` with `a₁=Dφ(1,0)`, `a₂=Dφ(0,1)` independent — the linear part maps the two open half-planes to the two open half-planes, and the cubic transverse term dominates the `O(s⁴)` correction. So at equal tangential level the arms differ transversally by `2|s|³ ≠ 0`: they peel apart like `y = ±x^{3/2}` and **cannot cross locally**. Hence the coarse CPs need no densification at a VP — there is no self-weave to suppress. (The escape hatches are all non-generic for a fixed view: a degenerate cusp keeps an *odd* transverse power — the rhamphoid `(s²,s⁵)` still has opposite sides — so genuine same-side crossing requires a *self-tangency of two distinct fold branches*, i.e. a degenerate node/lips, codimension ≥ 1.) The spurious grazes that *do* occur near a cusp are between the CC and a **different** branch passing close by (a near self-tangency, or a third sheet in the cusp's 3-fold pocket); those are addressed at the break-detection level, not by resampling the cusp.

**BC (boundary curve).** At each endpoint SP of kind `bcp`, find the unique CC ending at that BCP whose outgoing image-space tangent has positive dot product with the BC's outgoing tangent (the "tangent-aligned" CC at the BCP). Over a **matched region** extending from the BCP out to `min(L_BC / 2, L_CC / 2)` — whichever half comes first — the BC's samples **are** the CC's CP xy-arclengths from the BCP, *replacing* (not unioning) the BC's own raw vertices in that region. Beyond the matched region — the **un-matched middle** — the BC reverts to its own raw CPs (mesh-boundary vertices crossed by the chain). The losing BC arc (whose tangent is antiparallel to the CC's, or any endpoint with no positive-aligned CC) keeps only its own raw CPs.

The point of *replacement* rather than union: at a BCP the BC and the tangent CC are near-coincident in the image, so sampling them at *identical* arclengths-from-the-shared-origin makes their polylines coincide and they cannot weave. The old union kept the BC's own vertices inside the matched region, where they bulged off the CC's chord and produced spurious grazing crossings. For a **lune** (a BC sharing *both* endpoints with one CC, near-equal lengths — e.g. a boundary arc hugging the apparent contour between two consecutive BCPs) the two matched halves meet at the midpoint, so the BC becomes a vertex-exact copy of the CC over its whole span.
