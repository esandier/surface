...
class Surface:
    ...
    def plot_for_browser(self):
        ...
        lines = defaultdict(list)    # clé = visibilité, valeur = liste de lignes 2D [x,y]
        lines_3d = defaultdict(list) # même structure, points 3D [x,y,z]
        for i, l in enumerate(self.lines):
            type = "?"
            if l[0]["type"] == "??":
                continue
            ...
            for j in range(len(l) - 1):
                p, q = l[j]["fp"], l[j + 1]["fp"]
                pt3d = np.array(self.S(*p))
                line.append(self.XY(pt3d))
                line_3d.append(pt3d.tolist())
                for s, v in self.line_bks[i][j]:
                    split3d = np.array(self.S(*((1 - s) * p + s * q)))
                    line.append(self.XY(split3d))
                    line_3d.append(split3d.tolist())
                    lines[vis].append(line)
                    lines_3d[vis].append(line_3d)
                    vis = vis + v
                    line = [line[-1]]
                    line_3d = [line_3d[-1]]
            last_pt = l[-1]["fp"]
            last3d = np.array(self.S(*last_pt))
            line.append(self.XY(last3d))
            line_3d.append(last3d.tolist())
            lines[vis].append(line)
            lines_3d[vis].append(line_3d)
        self.print("[%0.3fs] %s" % (time.perf_counter() - t0, "préparation du dessin"))

        return lines, lines_3d
