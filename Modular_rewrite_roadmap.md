...
Debug-panel keys passed through the `debug` POST dict and routed in `views._debug_kwargs`:
- **`build_outline` kwargs:** `PROPAGATION ∈ {BFS, LP1, LP4}` (default LP4), `NEWTON_CONTOUR_POINTS` (default True — Newton-refine contour points), `REFINE_CUSPS` (default **False** — Newton-bisection refine VPs; see O4), `PROJECT_RESAMPLED` (default False), `RESOLUTION` (mesh + outline density).
- **scoped `surface_play.settings` overrides** (read deep in the outline phase): `DENSIFY_SUBDIV` (BC/HC densification, default 10), `HA_CUSP_TRIM` (default 5), `VP_TRIM` (per-branch cusp trim, default **1**), `VP_MATCH_MODE ∈ {match, trim}` (default **trim**; see O14 §5a).
- **construction kwargs:** `RESOLUTION` (→ mesh `resolution`), `JITTER`, `SEED`.

Module-level defaults in `surface_play/settings.py`: `RESOLUTION=210`, `DISPLAY_RESOLUTION=80`, `VP_MATCH_MODE="trim"`, `VP_TRIM=1`, `DENSIFY_SUBDIV=10`, `HA_CUSP_TRIM=5`, `PROJECT_RESAMPLED=False`. In production `DEBUG=False` hides the debug panel, so requests carry no overrides and these module defaults apply — hence the prod fast path (trim + no cusp refinement) is set by these defaults, not by the panel. No module reads `settings.py` directly except via these documented paths.
...
| **G21** | SIC resampled as one curve of DPs (no preimage extraction) | A SIC is ONE curve (spec lines 219/231): its two preimages are deduced per-DP via `flip`, **not** extracted as resampled polylines. `resample_all` builds the arclength skeleton from the DP/SP **`xyz`** (sheet-independent — `S(uv1) = S(uv2)`, so it is image-continuous regardless of `flip` alternation, eliminating the spurious-long-segment failure mode). Each resampled sample is itself a DP: within a SIS, interpolate **both** preimages close-aware via `flip` (`preimage-A` through `dps[p].uv1`, `preimage-B` through `dps[p].uv2`), then `xyz = S(preimage-A)`. SP endpoints are sourced **structurally** from their SIS+SPT-bary (`dp_pair_to_sis`/`sp_to_sis`): an SP at bary 0/1 resolves to its DP and the owning segment is the DP-pair SIS; an interior SP keeps its own SIS. **No proximity heuristic, no `uv`/`uv_alt` sheet-picking, no continuous preimage polyline.** (The earlier proximity walk hopped sheets where the two preimages passed near each other in uv — the root of the Boy/Enneper spurious segments.) `sp.uv_alt` is now vestigial for resampling (it is still written by O8/O11 and read by `_sic_samples` for HCs). TPs have 3 preimages, naturally handled since each TP-incident SubCurve resolves the owning SIS from its neighbour DP. |
...
```
**Algorithm:**
1. **Initial detection (spec lines 343-361):** for each CC chain, take the `d` field at consecutive CPs along the chain. Sign change in `dot(d[i], d[i+1])` flags a VP between them. Initial VP location = midpoint of the CS between the two CPs.
2. **Newton-bisection refinement — OPTIONAL (`refine` arg; pipeline default OFF since 2026-06-17).** When on: at each candidate take the midpoint, run Newton on the line orthogonal to the CS to find a true CP nearby, keep the half still bracketing the `d` sign change, iterate to `tol=1e-10`. This places the VP at the **exact cusp** (projected-contour velocity vanishes; equals the `d`-reversal point to machine precision) and sets the VP's `s` to the refined location's projection onto the native CS chord (not hardcoded `0.5`). Machine-precision cusps are required by O14's **equal-distance** VP matching (`VP_MATCH_MODE="match"`), so `refine` and `match` go together. The production default is `VP_MATCH_MODE="trim"` + `refine_cusps=False` (the perf fast path): the VP is just the **CS midpoint** (`refine=False`, `s=0.5`) and O14's asymmetric near-cusp trim removes the straddle without needing an exact cusp. `build_outline(refine_cusps=...)` wires this; the debug panel exposes it as **Refine cusps** (auto-unchecked when **VP match mode** = trim). The cusp-velocity-zero is a Whitney cusp, diffeomorphism-stable (Reorg §"No special resampling at VPs"), so the two arms can't cross regardless — refinement only sharpens the VP, it isn't needed for correctness in trim mode.
...
2a. **Delete degenerate SubCurves (2026-06-17).** Any SubCurve whose projected polyline is shorter than `1e-4 · ell` is collapsed to ~a single image point (e.g. a tiny CC loop at a bump tip under some jitter+view, or a non-generic axis-aligned view). Such subs are **dropped from the pipeline** — no `ResampledCurve` is emitted (incl. tiny SP-less closed loops). Rationale: they carry no visible line, their tiny `L` would poison the per-SP `delta` of neighbours sharing their SPs (`_sample_arclengths` would iterate 10⁷+ times climbing back to `ell`), and their near-coincident points blow up the client cubic-Bézier fit into image spikes (the fit's least-squares `det → 0`). Their tiny `L` is also excluded from the per-SP `delta` computation. History: originally a hard `raise` (aborted the whole outline → intermittent HTTP 400 on bumpy surfaces), then a verbatim pass-through, now deletion.
...
5. **BC/CC interpolation.** The sampled points usually lie between two points of the original curve. For BC/CC the `(uv)` coordinates of these points are interpolated, after `close`, and the projected image of the result is taken as the `xy` of the interpolated point.
5a. **VP cusp matching (`_cc_vp_match_targets` dispatcher, `settings.VP_MATCH_MODE`).** At a cusp the two CC half-curves are tangent and overlap in the image near the tip; independently sampled, their near-tip polylines spuriously cross and `compute_projection_breaks` emits a phantom inter-branch occlusion break (symptom: a contour that should hide half-way renders fully visible — the Klein bug). For each **VP shared by exactly two CC half-curves**, the arclength ladder is overridden near the cusp by one of two algorithms:
   - **`"match"`** (`_cc_vp_match_equidist`, 2026-06-16) — equal-distance resampling. The **shorter** branch is *master*; the **longer** branch is resampled so its points sit at the **same Euclidean distances to the VP** as the master's, over the master's reach (interpolate `uv` along the longer polyline and **reproject** `S(uv)→XY`; vectorized distance-bisection with batched `surface.S`). The longer branch's replaced region is the **contiguous near-VP arclength run** (walk from the VP until distance first exceeds reach — not every vertex within Euclidean reach, which would drop distant loop-backs and leave a long skip segment). Then the innermost `VP_TRIM` points are dropped from both branches. **Requires the machine-precision VP from O4** (`refine_cusps=True`), since distances are measured from it.
   - **`"trim"`** (`_cc_vp_match_asym`, **default since 2026-06-17**) — asymmetric near-cusp trim, **no resampling/reprojection**. The *reference* branch (the one whose `VP_TRIM`-th-from-VP vertex is **farther** from the VP in the image) drops exactly `VP_TRIM` vertices nearest the cusp; the *matched* branch drops as many near-VP vertices as makes its first surviving vertex's image distance to the VP closest to the reference's (so both leave the cusp at the same radius — no straddle). Works with the unrefined CS-midpoint VP (`refine_cusps=False`), giving the perf fast path (skips O4's `_refine_cusp`, the dominant VP-heavy cost). Each VP end handled independently.

   `VP_TRIM` (settings, **default 1**; debug-panel `VP trim`) is the per-branch trim count. `VP_MATCH_MODE` is debug-panel-selectable (`VP match mode`).
6. **SIC = one curve of DPs (spec §SIC, lines 219, 231).** An SIC is stored as ONE curve; its two domain preimages are **not** extracted as resampled polylines — they are deduced per-DP via `flip`. Implementation (`_sic_build` + the `sub.kind == "SIC"` branch of `resample_all`):
   - **Arclength skeleton = the DP/SP `xyz` polyline.** A DP's `xyz` is sheet-independent (`S(uv1) = S(uv2)`), so the skeleton is single-valued and image-continuous **regardless of how `flip` alternates** along the chain. This is what makes the SIC behave like a BC/CC and removes the spurious-long-segment failure mode by construction.
   - **Each sample is itself a DP.** For a target falling in polyline segment k (one SIS `s`, flip `f`), interpolate **both** preimages close-aware on `s` — `preimage-A` through `dps[p].uv1`, `preimage-B` through `dps[p].uv2`, with `f` selecting the q-end sheet — at the segment-local bary. `xyz = S(preimage-A)` lifts the sample on-surface. No sheet is ever chosen by proximity, and no continuous preimage curve is materialised.
   - **SP endpoints are sourced from their SIS, not from `sp.uv`/`uv_alt`.** An SP sits on a known SIS at a known SPT bary; an SP at bary 0/1 is resolved to its DP (the owning segment is then the DP-pair SIS), an interior SP keeps its own SIS. The owning SIS and barys are found **structurally** from `dp_pair_to_sis` / `sp_to_sis` (no proximity). `rc.uv` stores `preimage-A`; nothing downstream reads SIC `uv` (visibility/render use `xy`/`depth`), so it is metadata.
...
4. **SIC = one curve of DPs (G21):** consecutive `xy` steps on a SIC RC are bounded (no preimage jump → no spurious long segment); and each interior sample's stored `uv` lifts (via `S` then `XY`) back to its rendered `xy` (the sample is an on-surface DP). Domain `uv` may legitimately jump at a DP node — only `xy`/`xyz` continuity is asserted.
