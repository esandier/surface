...
2. **Newton-bisection refinement — MANDATORY (default `refine=True`, 2026-06-16):** at each candidate, take the midpoint, run Newton on the line orthogonal to the CS to find a true CP nearby, keep the half still bracketing the `d` sign change, iterate to `tol=1e-10`. This places the VP at the **exact cusp** — the point where the projected-contour velocity vanishes (verified equal to the `d`-reversal point to machine precision). Machine-precision cusps are required by the equal-distance VP matching in O14 (distances are measured *from* the VP, so it must be exact). The VP's `s` (parametric position on its CS) is set to the refined location's projection onto the native CS chord (no longer hardcoded `0.5`), so split_ccs_at_vps' SPT bary stays consistent with the SP geometry.
...
5a. **Equal-distance VP matching + cusp trim (2026-06-16, `_cc_vp_match_targets`).** At a cusp the two contour branches are tangent and overlap in the image near the tip; with independent sampling their polylines spuriously cross, and `compute_projection_breaks` then emits a phantom inter-branch occlusion break (the symptom: a contour that should hide half-way renders fully visible). Fix: for each **VP shared by exactly two CC half-curves**, pick the **shorter** branch (projected length) as *master*; resample the **longer** branch so its points sit at the **same Euclidean distances to the VP** (image space) as the master's, over the master's reach (beyond that the longer branch keeps its native CP arclengths). The matched arclengths are found by interpolating `uv` along the longer branch's polyline and **reprojecting** `S(uv)→XY` (vectorized bisection on the distance, batched `surface.S` per step), so matched points lie on the contour. The longer branch's replaced region is the **contiguous near-VP arclength run** (walk from the VP until distance first exceeds reach — *not* every vertex within Euclidean reach, which would drop distant loop-backs and leave a long skip segment). Finally the innermost **`VP_TRIM`** points (settings, default 3; debug-panel `VP trim`) are dropped from **both** branches: near the velocity-zero cusp, sampling is hypersensitive and the two innermost points straddle the VP — trimming removes the straddle while keeping the branches matched. Each VP end is handled independently (a branch may be master at one end, longer at the other); a non-monotone branch within reach falls back to native sampling. Requires the machine-precision VP from O4 (distances are measured from it).
